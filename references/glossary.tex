...
\newglossaryentry{dvcs}{
  name = {DVCS},
  description = {
    (distributed version control system) is a class of version control
    software that allows developers to view modifications to source code over
    time (through commits, changes to blocks of lines). It differs from a
    centralised version control system in that each copy checked out by a
    developer is in itself a repository}
}

\newglossaryentry{github}{
  name = {GitHub},
  description = {
    is an online service allowing developers to collaborate using Git, a
    \gls{dvcs}. The service's primary use is for free and open source
    software, though it also offers paid accounts for private repositories}
}

\newglossaryentry{orm}{
  name = {object-relational mapping},
  description = {
    is a layer between an application's code and a database. It allows
    developers to interact with a database without writing code specific to
    one database engine (MySQL, PostgreSQL, Oracle, etc), increasing
    flexibility}
}
...
\newglossaryentry{cookie}{
  name = {cookies},
  description = {
    are small amounts of text data stored in a web browser and transmitted to
    a web server on every page request}
}
...
\newacronym{iis}{IIS}{irreducible inconsistent subsystem}
...
\newacronym{sipig}{SIPIG}{Strategic Information Projects Implementation Group}
\newacronym{sql}{SQL}{structured query language}
\newacronym{sso}{SSO}{single sign-on}
\newacronym{utc}{UTC}{University Teaching Committee}
...
% Also had name sign-off from SR, SW

\newacronym{cm}{CM}{Claire McNamara}
\newacronym{lc}{LC}{Laura Crossley}
\newacronym{mh}{MH}{Matt Hanson} % [@todo need to check I can use his name]
\newacronym{sa}{SA}{Steve Ashby}
